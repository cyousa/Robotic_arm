...
float Iq_Target=0.05,Id_Target=0;
...
void Klark_change()//克拉克变换
{
	...
	Ia=(((uint16_t)hadc1.Instance->JDR1 - 2048) / 4096.f) / 50.f / 0.005f;
	Ib=(((uint16_t)hadc1.Instance->JDR2 - 2048) / 4096.f) / 50.f / 0.005f;
	Ic=(((uint16_t)hadc1.Instance->JDR3 - 2048) / 4096.f) / 50.f / 0.005f;
...
float Iq_current_loop()
{
		...
		if(Iq_current.error_sum>60
			)
		{
				Iq_current.error_sum=60;
		}
		else if(Iq_current.error_sum<-60)
		{
				Iq_current.error_sum=-60;
		}
